...
export default function ActeurDashboard() {
  ...
  useEffect(() => {
    async function charger() {
      ...
      const { data: demandes } = await supabase
        .from("demandes")
        .select("*, services (titre, domaine), profiles:client_id (nom, telephone, region, departement)")
        .eq("acteur_id", user.id)
        .order("created_at", { ascending: false });
  ...
  async function ajouterService(e) {
    ...
    const { data, error } = await supabase
      .from("services")
      .insert({ acteur_id: userId, type, domaine, titre, description })
      .select()
      .single();
        ...
        <div className="grid md:grid-cols-2 gap-4">
          {demandesRecues.map((d) => (
            <div key={d.id} className="border border-ink/10 rounded-2xl p-5">
              ...
              {d.profiles?.telephone && (
                <a
                  href={`tel:${d.profiles.telephone}`}
              ...
              <select
                ...
                className="mt-3 block border border-ink/20 rounded-lg px-3 py-2 text-sm bg-white"
              >
